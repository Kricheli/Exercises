...
//1 getAgencyByName
      ...
      {
              return carMarket.sellers.find((element) => 
              (element.agencyName === name))
      }
...
//2 getAgencyIdByName
       carMarket.getAgencyIdByName = function (string)
       {
               const agency = this.sellers.find((element) => {return element.agencyName === string});
...
//3 getAllAgenciesByName
     carMarket.getAllAgenciesByName = function ()
     {
       let agency = carMarket.sellers.map((type) => 
       {
         return type.agencyName
       });
       return agency;
     }
     console.log(carMarket.getAllAgenciesByName());
...
//5
carMarket.getAllCarsToBuyByAgencyId = function (idOfAgency)
{
  const agencyCars = [];

  const findAgencyId = carMarket.sellers.find((id) =>
  {
    if(id.agencyId === idOfAgency)
  {
      id.cars.forEach((car) =>

      car.models.forEach((model) => 

      agencyCars.push(model)
  ))}
  });

  return agencyCars;
}
console.log(carMarket.getAllCarsToBuyByAgencyId('Plyq5M5AZ'));
...
//6
        carMarket.getAllBrandsToBuyAgencyId = function (idOfAgency)
        {
          const findAgencyId = carMarket.sellers.find((id) => {return id.agencyId === idOfAgency})
          return findAgencyId && findAgencyId.cars;
        };
...
//7
         ...
         {
           return carMarket.customers.find((userInput) => userInput.name === customerName)
         }
...
//8
        ...
        {
           const findCustomerName = carMarket.customers.find((name) => {return name.name === customerName});
           return findCustomerName && findCustomerName.id;
        }
...
{
  return carMarket.customers.map((customerName) => customerName.name)
}
...
{
 const carsArr = carMarket.customers.find((id) =>{return id.id === customerId});
 return carsArr && carsArr.cars
};
...
console.log('---------- 11 ------------------------------------------------------------------------------------------');
      ...
      //* getCustomerCash
      //? @param {id} - costumerId - costumer i
...
//11
  ...
  {
    const customerCars = carMarket.customers.find((id) => {return id.id === customerId});
    return customerCars && customerCars.cash;
  }
...
//12
  ...
  {
    const branding = carMarket.sellers.forEach((uno) => uno.cars.forEach((test) => test.)
    return branding;
    )}
